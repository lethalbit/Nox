...
The `rm` message seems to always be standalone and has a 16 byte payload associated with it.

### The `IDevAnalyzerControl1029` Interface

The following calls have been seen with the `IDevAnalyzerControl1029` interface. This interface is occasionally prefixed with the `ln` message.
...
#### `armStartMesurments`
#### `armStopMesurments`
#### `setContinuousMesurmentInterval`
#### `setSamplingInterval`
