...
`PALOAD LENGTH` looks to be the length of the whole transaction payload minus 4 bytes. This means that a transaction can span more than one packet, in that case the Ethernet, IP, and TCP headers are still applied, but the N2X Transport header is not and the remainder is just filled with payload data.
